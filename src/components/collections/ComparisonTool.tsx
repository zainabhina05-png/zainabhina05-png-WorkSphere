    ...
    async function fetchFolders() {
      try {
        ...
        if (data.folders) {
          ...
          setFolders(
            data.folders.filter((f: any) => f.id !== currentFolder.id),
          );
        }
      } catch (e) {
    ...
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm mb-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-white flex items-center gap-2">
          <ArrowRightLeft className="w-5 h-5 text-indigo-500" /> Compare
          Collections
        </h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-zinc-100 text-sm rounded-xl focus:ring-[var(--primary-accent)] focus:border-[var(--primary-accent)] block w-48 p-2.5"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
        ...
        {loading ? (
          ...
          <MetricsWidget
            venues={compareFolder.venues}
            title={compareFolder.name}
            isCompact
          />
        ) : (
